...
async function isRecipeWithGemini(description, title) {
    ...
    2. PAYS : Choisis UN pays dans cette liste : France, Italie, Espagne, Grèce, Liban, USA, Mexique, Orient, Asie.
    Si le pays d'origine est évident mais n'est pas dans la liste, utilise le plus proche ou "Asie". 
    ...
    for (const model of models) {
        try {
            ...
            if (!r.ok) {
                if (r.status === 429) {
                    console.log(`   🚨 Quota dépassé pour ${model} (429).`);
                    lastError = 'QUOTA_EXCEEDED';
                } else {
                    console.log(`   ⚠️ Erreur Gemini (${model}) : Status ${r.status}`);
                }
            ...
            console.log(`    🔸 Réponse raw (${model}):`, text.length > 100 ? text.substring(0, 100) + '...' : text);
            console.log(`   🔸 Réponse raw (${model}): ${text.substring(0, 50)}...`);
            const match = text.match(/\{[\s\S]*\}/);
            if (match) {
                const parsed = JSON.parse(match[0]);
                if (parsed.isRecipe) {
                    console.log(`   ✅ Succès avec ${model} !`);
                    // Robustesse : certains modèles peuvent retourner 'title' ou 'name' au lieu de 'recipeName'
                ...
                console.log(`   ⚠️ Pas de JSON trouvé dans la réponse de ${model}.`);
            }
        } catch (e) {
            console.error(`   ❌ Exception ${model}:`, e.message);
        }
    }
    return lastError === 'QUOTA_EXCEEDED' ? { isRecipe: true, isQuotaExceeded: true } : null;
}
...
async function checkWordPressDuplicate(videoUrl) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            // Filtrage manuel pour éviter les faux positifs (ex: ID contenu dans un autre texte)
            const found = posts.some(p => p.link && p.id);
            if (found) {

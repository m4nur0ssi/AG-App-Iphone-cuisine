...
async function syncRecipes() {
    ...
    const initialTotalPages = SYNC_ALL ? 10 : 1; // Uniquement 1 page en mode rapide si !SYNC_ALL
    let page = 1;
    let totalPages = initialTotalPages; // Will be updated by API response if SYNC_ALL is true
    let posts = [];
    ...
    do {
        ...
        try {
            ...
            const testUrl = `http://${ACTIVE_IP}/wordpress/wp-json/wp/v2/posts?per_page=50&page=${page}&status=publish&_embed&orderby=modified&nocache=${Date.now()}&v=${Math.random()}`;
            ...
            const pageRecipes = cleanPosts.map((post, index) => {
                ...
                const specialMapping = [
                    ...
                    { tag: 'Famille', keywords: ['famille', 'enfants', 'kids', '#famille'] }
                ];

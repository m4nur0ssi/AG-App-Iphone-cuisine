...
export function getIngredientVisual(name: string): string | null {
    ...
    // Fusionner les dictionnaires pour avoir le plus de chances
    const marmitonDict = { 
    ...
    for (const key of allKeys) {
        if (normalize(key) === normCleanName) {
            const visual = marmitonDict[key];
            return visual === 'no-image' ? null : visual;
        }
    }

    const quoteHandler = (str: string) => str.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

    // 2. Recherche par inclusion de MOT ENTIER
    const sortedKeys = allKeys.sort((a, b) => b.length - a.length);
    for (const key of sortedKeys) {
        const normKey = normalize(key);
        if (normKey.length < 3) continue; // Évite les faux positifs sur les mots trop courts
        ...
        try {
            ...
            continue; // Skip invalid patterns if any
        }
    }

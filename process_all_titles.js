...
// Remove duplicate .jpg files if .png exists
['express', 'healthy'].forEach(name => {
    const jpgPath = path.join(themesDir, `${name}.jpg`);
    const pngPath = path.join(themesDir, `${name}.png`);
    if (fs.existsSync(jpgPath) && fs.existsSync(pngPath)) {
        console.log(`Deleting duplicate ${name}.jpg`);
...
async function processImages() {
    for (const item of filesToProcess) {
        ...
        try {
            ...
            if (item.long) {
                fontSizeRatio = 0.055; // Smaller for long texts
            }
            if (item.text.length > 25) {
                fontSizeRatio = 0.045; // Even smaller for very long texts
            }

            const fontSize = Math.floor(width * fontSizeRatio);
            const letterSpacing = Math.floor(width * 0.02);
